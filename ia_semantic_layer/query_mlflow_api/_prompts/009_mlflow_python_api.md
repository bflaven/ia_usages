...
## PROMPT
As a Python and MLflow expert, the script is working so keep it as it `0008_mlflow_python_api.py`but it works only for one item. Can you make the following modification :
1. Can you loop through an object named `gold_dataset": []` from a json file named `gold_dataset_api_mlflow_full_2.json`. Below you can find a sample of the json structure.
2. The script should load the following values contained in the json : `lang`, `cms_section_keywords_list`, `content` to populate the values in the variables of the script as lang`, `cms_section_keywords_list`, `content`.
3. At the end of the loop can add each run for each item into a single object output in a json like it is the case now.

`gold_dataset_api_mlflow_full_2.json`
```json
{
    "gold_dataset": [
        {
            "source": "011_ner_RFI_FR_journal_des_sports_afrique_MZ534734",
            "prompt_template": "En vous inspirant... etc",
            "lang": "français",
            "cms_section_keywords_list": "Sports, Économie / Technologie... etc",
            "content": "les sports ce matin avec antoine grenier bonjour antoine... etc"
        },
        {
            "source": "008_ner_RFI_FR_grand_invite_Afrique_MZ535421",
            "prompt_template": "En vous inspirant... etc",
            "lang": "français",
            "cms_section_keywords_list": "Sports... etc",
            "content": "Bonjour Aona Eziak-Onwa.\nVous étiez à Bruxelles cette semaine... etc"
        }
    ]
}
```

`0008_mlflow_python_api.py`
```python
#!/usr/bin/python
# -*- coding: utf-8 -*-
...
def main():
    ...
    GATEWAY_ENDPOINT_NAME = "[model-name]"
