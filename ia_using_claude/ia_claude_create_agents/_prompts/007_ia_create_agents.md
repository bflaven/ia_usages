...
As a python expert, can you write a script that enable these two files `claude_seo_agents.py` and `claude_agent_framework.py` to work together given below. You can still keep best practises for coding e.g Class and function but you can merge these functions and class into a single file named "using_claude_seo_agents.py" then show how to launch a test to retrieve from an real example on a "subject" e.g "the pope Francis death". Follow these guideleines: 

1. Make the "subject" a variable in the script
2. Replace ANTHROPIC_API_KEY by CLAUDE_API_KEY.
3. The CLAUDE_API_KEY is stored inside a .env so be sure to use 
...
4. Show in a example how to load the function `KeywordResearchAgent` and `ContentBriefAgent`


**claude_seo_agents.py**
...
I'll help you merge these two files into a single script that follows the guidelines you've provided. Let me create a unified script called "using_claude_seo_agents.py" that incorporates both files and shows how to use the agents with an example.

I've created a unified Python script called `using_claude_seo_agents.py` that combines both files according to your requirements. Here's an explanation of how I implemented the requested changes:

1. **Combined both files into one script**: I merged the functionality from both `claude_seo_agents.py` and `claude_agent_framework.py` into a single file.

2. **Made "subject" a variable**: I created a `subject` variable in the example code section that can be easily modified.

3. **Replaced API key variable**: Changed `ANTHROPIC_API_KEY` to `CLAUDE_API_KEY` throughout the code.

4. **Added environment variable loading**: Included `load_dotenv()` to properly load the API key from a `.env` file.

5. **Fixed a bug in the original code**: In the `call_claude` method, I moved the line that adds the user message to `messages` before creating the request data.

6. **Added example usage**: Created a comprehensive example that shows how to:
   - Initialize both `KeywordResearchAgent` and `ContentBriefAgent`
   - Set up a workflow with both agents
   - Execute the workflow on the example subject ("the pope Francis death")
   - Print useful summary information about the results
...
3. Run the script:
   ```
   python using_claude_seo_agents.py
   ```

You can change the `subject` variable in the script to research different topics. The example shows how the two agents work together in sequence, with the keyword research results feeding into the content brief creation process.
...
if __name__ == "__main__":
    ...
    print(f"Starting SEO workflow for subject: {subject}")
