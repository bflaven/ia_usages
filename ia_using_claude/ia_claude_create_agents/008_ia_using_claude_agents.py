...
# launch the file
python 008_ia_using_claude_agents.py
...
# Constants
DEFAULT_MODEL = "claude-3-sonnet-20240229"  # Using a more widely available model
...
if __name__ == "__main__":
    ...
    try:
        ...
        pkg_resources.require(['requests', 'python-dotenv'])
        print("Required packages found: requests, python-dotenv")
    ...
    try:
        # Initialize agents with a specific model (using a more widely available model)
        keyword_agent = KeywordResearchAgent(model="claude-3-sonnet-20240229")
        content_agent = ContentBriefAgent(model="claude-3-sonnet-20240229")
        
        # Test a simple API call to ensure authentication is working
        print("Testing API connection with a simple call...")
        test_response = keyword_agent.call_claude("This is a test message to verify API connectivity.")
        print("API test successful! Claude responded.")
        print("-" * 50)
        ...
        print("1. Make sure your .env file has CLAUDE_API_KEY=your_key_here (without quotes)")
        print("2. Check that your API key is valid and has not expired")
        print("3. Ensure you have the required packages installed: requests, python-dotenv")
        print("4. Try with a different Claude model (current: claude-3-sonnet-20240229)")
