...
- **Feature**: **Export JSON (button 4)** — same selection logic, same columns, downloads as a pretty-printed JSON array; filename: `breadcrumb_migration_bulk_description_YYYYMMDD_HHMMSS.json`
- **Clarification — ↺ Sync from WordPress**: reads the live WordPress tag description for every approved proposal and writes it back into `proposed_description` in `wp_breadcrumb_proposals`; rows where the description was manually written (`proposed_description ≠ wikidata_description`) are skipped to protect hand-crafted content; use this after editing tag descriptions directly in WordPress to keep the plugin DB in sync
- **Clarification — Save to WordPress**: copies the Wikidata description (or actual description for ✍ Written rows) from the plugin DB → live WordPress tag Description field for all checked rows; operates on selected rows only; triggers a confirm dialog before writing
- `admin-page.php`: both `bm-bulk-desc-actions-top` and `bm-bulk-desc-actions-bottom` rewritten to 4-button flex layout; button labels shortened ("Save to WordPress", "↺ Sync from WordPress"); brief hint `<p class="bm-bulk-actions-hint">` appended (wraps below buttons via `flex: 0 0 100%`); bottom bar gains second sync button (`#bm-sync-descriptions-bottom`) and both export buttons
- `admin.js`: `bmBulkDescGetSelectedRows()` — returns checked rows or falls back to all visible rows; `bmBulkDescRowToObject()` — extracts 7-field object per row from DOM (handles Empty `<em>` sentinel); `bmTriggerDownload()` — Blob + anchor click helper; `bmExportTimestamp()` — `YYYYMMDD_HHMMSS` string; `.bm-btn-export-csv/json` click handlers; sync handler updated to respond to both `#bm-sync-descriptions` and `#bm-sync-descriptions-bottom`
- `admin.css`: `.bm-bulk-actions-hint` — `flex: 0 0 100%` forces the hint to wrap below all buttons in the same flex container
