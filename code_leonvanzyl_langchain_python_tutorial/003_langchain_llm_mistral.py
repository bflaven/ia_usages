...
# LAUNCH the file
python 003_langchain_llm_mistral.py
...
llm = Ollama(model=llm_model)
prompt = ChatPromptTemplate.from_messages([
    ("system", "You are AI chef. Create a unique recipe based on the following ingredients"),
    ("human", "{ingredients}")
])
# create a LLM chain
chain = prompt | llm
# Providing a string instead of a dictionary
# response = chain.invoke({"subject": "scientist"})
response = chain.invoke({"ingredients": "tomatoes"})
# ouput
print (response)
