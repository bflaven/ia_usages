...
function bf_re_emb_save_meta_box( $post_id ) {
    ...
    // Add new
    if ( ! empty( $_POST['bf_related_posts'] ) ) {
        $related_ids = array_map( 'intval', $_POST['bf_related_posts'] );
        $rank = 1;
...
function bf_re_emb_render_meta_box( WP_Post $post ) {
    ...
    <div id="bf-related-posts-wrap">
        <p>Select related posts (drag to reorder):</p>
        ...
        <ul id="bf-related-posts-list" style="list-style: none; padding: 0; margin: 0 0 10px 0;">
            ...
            ?>
            <li style="padding: 5px; margin: 3px 0; background: #f0f0f0; cursor: move; border-left: 3px solid #2271b1;">
                <input type="hidden" name="bf_related_posts[]" value="<?php echo esc_attr( $rel_post->ID ); ?>">
                ...
                <button type="button" class="bf-remove-related" style="float: right; color: #b32d2e; background: none; border: none; cursor: pointer;" title="Remove">×</button>
            </li>
            <?php endforeach; ?>
        </ul>
    ...
    jQuery(document).ready(function($) {
        $('#bf-related-posts-list').sortable({ placeholder: 'ui-state-highlight' });
        
        $(document).on('click', '.bf-remove-related', function() {
        ...
        $(document).on('click', '.bf-search-result', function() {
            ...
            if ($('input[name="bf_related_posts[]"][value="' + postId + '"]').length) {
                alert('Already added');
                return;
            }
            
            var html = '<li style="padding: 5px; margin: 3px 0; background: #f0f0f0; cursor: move; border-left: 3px solid #2271b1;">';
            html += '<input type="hidden" name="bf_related_posts[]" value="' + postId + '">';
            html += '<span class="dashicons dashicons-menu" style="color: #999;"></span>';
            html += '<strong>#' + postId + '</strong> — ' + postTitle;
            html += '<button type="button" class="bf-remove-related" style="float: right; color: #b32d2e; background: none; border: none; cursor: pointer;">×</button>';
            html += '</li>';
