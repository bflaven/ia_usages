...
function bf_re_emb_save_meta_box( $post_id ) {
    ...
    // Get ordered IDs (user can reorder in both modes)
    ...
    if ( $is_custom ) {
        ...
        // Add selected only in order
        $rank = 1;
        foreach ( $ordered_ids as $related_id ) {
            // Only add if it's in selected_ids
            if ( in_array( $related_id, $selected_ids ) && $related_id !== $post_id ) {
                $wpdb->insert(
                    $table_name,
                    array(
                        'post_id'         => $post_id,
        ...
        // Default mode - save order for all posts
...
function bf_re_emb_render_meta_box( WP_Post $post ) {
    ...
    jQuery(document).ready(function($) {
        ...
        // Reset button
        $('#bf-reset-selection').on('click', function() {
            if (confirm('Reset to default? This will show all posts from CSV/database.')) {
                $('#bf-custom-selection').prop('checked', false).trigger('change');
                location.reload();
            }
        });
...
function bf_re_emb_render_settings_page() {
    ...
    <div class="wrap">
        ...
        <div class="card">
            ...
            <pre style="background: #f5f5f5; padding: 15px; border: 1px solid #ddd; overflow-x: auto; font-family: monospace; line-height: 1.6;">$related = bf_get_related_posts( $post_id, $limit );
