...
It is not working 
1. When I edit a tag and choose "Custom selection" and save it I need to reload the page to see the "Custom selection" selection. 
2. I can reorder the "Custom selection" and save it even when I relaod.

3. The reset button is OK as when I reset I got the list back again. Below an example of URL when I edit a tag

- by default CASE_1
```text
Family ID: 72
#32 — Famille Diallo Sim: 0.998 | Uses: 3 | O✓
#31 — Diallo Guidileye Sim: 0.966 | Uses: 1 | PERSON✓
#33 — Diallo Fadima Touré Sim: 0.761 | Uses: 1 | PERSON✓
#34 — Diallo Fatoumata Camara Sim: 0.756 | Uses: 1 | PERSON
#35 — Diallo Telli Sim: 0.745 | Uses: 1 | PERSON
```
- with "Custom selection" CASE_2
```text
Family ID: 72
#33 — Diallo Fadima Touré Sim: 0.761 | Uses: 1 | PERSON✓
#34 — Diallo Fatoumata Camara Sim: 0.756 | Uses: 1 | PERSON✓
...
- with "Custom selection" with reorder CASE_3
```text
Family ID: 72
#35 — Diallo Telli Sim: 0.745 | Uses: 1 | PERSON✓
#34 — Diallo Fatoumata Camara Sim: 0.756 | Uses: 1 | PERSON✓
...
When I click reset, I go back CASE_1

Ship the plugin with the uninstall and readme but no extra explanation.


## PROMPT_4
It is working but when I edit a tag and choose "Custom selection" and save it I need to reload the page to see the "Custom selection" selection can you fix this UX issue. The reset button is OK as when I reset i giot the list back again. Below an example of URL when I edit a tag. Can you fix this UX bug.
```text
http://localhost:8080/wp-admin/term.php?taxonomy=post_tag&tag_ID=30&post_type=post&wp_http_referer=/wp-admin/edit-tags.php?taxonomy=post_tag&post_type=post&s=Diallo
```
...
## PROMPT_3
Keep the plugin has it is but make that change: 
When I reset I still have the previous "Custom selection" e.g tags ids 1, 4, 30 so it does not work. Imagine the default tags ids list is : 43, 56, 9, 2, 5, 1, 4, 30. I made a "Custom selection" I go for 1, 4, 30 but then i want to undo this selection so I click on the reset button, it brings back the the default tags ids list is : 43, 56, 9, 2, 5, 1, 4, 30.
Tags ids are just given as example, it has to dynamic.
Ship the plugin with the uninstall and readme but no extra explanation.
...
## PROMPT_2
I have already the csv impported :
- Tag Relationships:  447
- Tag Families:   71


But there is only four tags in the tags, I am on fresh install made with docker with 4 items
...
## PROMPT_1
As a WordPress expert, can you write a plugin with best practices that enable to load into Wordpress the values from this CSV `related_tags_embeddings_settings_csv_1.csv`.

1. Based on the work that have been made for `bf_wp_related_embeddings_db.php`, create the same process with readme and uninstall. Make the best of the plugin and try to leverage on the information that is contained in the csv. Think of a great UX experience where I have a default and editorial choice with reset button that already explain. Enable to update content of mariadb wp database like you did for the other plugin. Take advantage of all the specifications I have already written.


2. Here the extract of the cvs produce
```csv
family_id,canonical_tag_id,canonical_label,tag_id,tag_label,similarity_to_canonical,usage_count,entity_label
0,2264,information,2264,information,1.0,3,O
0,2264,information,371,.info,0.9660738444578335,1,O
0,2264,information,1698,infosuroit,0.803888116401795,1,O
0,2264,information,3252,information landscape,0.7605102647502541,1,O
1,578,3WDOC,578,3WDOC,1.0,52,CARDINAL
```

3. The python is created by script in python. The script gives me:
