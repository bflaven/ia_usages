...
It does not work do I have to make some CSS changes please do it so I cut and paste the new css

```css
/* ===========================================================================
   SEMAPHORE DASHBOARD LAYOUT
   =========================================================================== */
/* Semaphore dashboard layout */
.semaphore-dashboard-wrap {
    max-width: 1200px;
}

.semaphore-dashboard-shell {
    display: flex;
    gap: 20px;
...
.semaphore-dashboard-main {
    flex: 1 1 0;
    min-width: 0;
}

.semaphore-dashboard-side {
    width: 280px;
    flex: 0 0 auto;
}
...
.semaphore-card-title {
    margin: 0 0 4px;
    font-size: 20px;
}

.semaphore-card-description {
    margin: 0;
    color: #555d66;
    font-size: 13px;
}

.semaphore-card-header-side {
    flex: 0 0 auto;
}
...
.semaphore-help-inner {
    padding: 12px 14px;
    font-size: 13px;
}

.semaphore-help-inner h3 {
    margin: 0 0 8px;
    font-size: 15px;
}

.semaphore-help-inner h4 {
    margin: 12px 0 4px;
    font-size: 13px;
}

.semaphore-help-inner ul {
    margin: 0 0 6px 18px;
    padding: 0;
    list-style: disc;
}
...
.semaphore-panel--left {
    flex: 1;
}

.semaphore-panel--right {
    flex: 1.4;
}

.semaphore-panel-block + .semaphore-panel-block {
    margin-top: 18px;
}

.semaphore-panel-title {
    margin: 0 0 2px;
    font-size: 15px;
}

.semaphore-panel-description {
    margin: 0 0 8px;
    font-size: 12px;
    color: #666;
}
...
.semaphore-table-box--families {
    max-height: 220px;
}

.semaphore-table-box--members {
    max-height: 220px;
}

/* Actions bar */
.semaphore-actions-bar {
    margin-top: 14px;
}

.semaphore-actions-bar .button + .button {
    margin-left: 6px;
}

.semaphore-actions-note {
    margin-top: 8px;
    font-size: 12px;
    color: #555;
}
...
.semaphore-side-links {
    margin: 0;
    padding-left: 18px;
    font-size: 12px;
}

.semaphore-side-links li {
    margin-bottom: 3px;
}
...
## PROMPT_14
For the CSS and stuff that is OK, but for the UX where are the 2 columns

1. Left column "All Tags", right "Tag Families" and below the column "Tag Families" the Family Members.

2. The btn "Show help" does not work at all.

3. Remove links for Documentation and Tag families concepts and CSV import format. In the same plugin say only that it leverages on Semaphore: Related Embeddings and Semaphore: Tag Families CSV. Be more specific on the logic of the plugin but keep it as User Storie example.
...
Can you rewrite the code below with these changes:
1. Make the UX function `semaphore_render_dashboard_page()` a better look and feel. For instance add behavieux like open and close to show help information as a user it is not useful to see always the help.

2. Take example on the layout of the "Post Duplicator Settings" cf https://wordpress.org/plugins/post-duplicator/. I have cut and paste the rendered HTML, do not use the same css class, take on only inspiration and créate css class and html code id required.
...
function semaphore_render_dashboard_page() {
    ...
    <div class="wrap">
        ...
        <p>Manage tag families as a clustering layer on top of your existing post tags. Existing post_tag assignments are not changed.</p>

        <!-- open/close if needed like a kind of dashboard-widgets -->
        <div class="bf-info-box">
            <strong>How to use this manager</strong>
            <br><br>
            <strong>All Tags (left column)</strong><br>
            – This lists all existing <code>post_tag</code> terms from WordPress.<br>
            – Click a row to select a tag (the row turns light green when selected).<br>
            – Selected tag = candidate to become a family, or to be attached to an existing family.<br>
            <br>
            <strong>Tag Families (top right column)</strong><br>
            – Shows all current families (one row per canonical tag / cluster).<br>
            – Click a family row to load its members in the “Family Members” list below (the active family row turns light green).<br>
            – You can search families by ID or label with the search field above the list.<br>
            <br>
            <strong>Family Members (bottom right column)</strong><br>
            – Lists tags currently attached to the selected family (cluster).<br>
            – Click a member row to select it (selected row is light green).<br>
            – Use “Detach selected member from current family” to remove that tag from the cluster (this does not delete the tag itself).<br>
            <br>
            <strong>Buttons</strong><br>
            – <em>Convert selected tag to family</em>: turns the selected tag in “All Tags” into a new family + canonical entry in the overlay table.<br>
            – <em>Attach selected tag to current family</em>: with a tag selected on the left and a family selected on the right, adds that tag as a member of the family.<br>
            – <em>Detach selected member from current family</em>: with a family and a member selected, removes that mapping from the family.<br>
            <br>
            Light green rows indicate the currently selected tag, family, or member that actions will apply to.
        </div>
        ...
        <div id="semaphore-families-layout" style="display:flex;gap:20px;align-items:flex-start;">
            ...
            <div style="flex:1;min-width:320px;">
                <h2>All Tags</h2>
                <input type="text" id="semaphore-tag-search" placeholder="Search tags by name or ID..." style="width:100%;max-width:100%;margin-bottom:10px;">
                <div id="semaphore-all-tags-table" style="border:1px solid #ddd;max-height:500px;overflow:auto;"></div>
            </div>

            <div style="flex:1.5;min-width:420px;">
                <h2>Tag Families</h2>
                <input type="text" id="semaphore-family-search" placeholder="Search families by ID or label..." style="width:100%;max-width:100%;margin-bottom:10px;">
                <div id="semaphore-families-list" style="border:1px solid #ddd;max-height:240px;overflow:auto;margin-bottom:15px;"></div>

                <h2>Family Members</h2>
                <div id="semaphore-family-members" style="border:1px solid #ddd;max-height:260px;overflow:auto;margin-bottom:10px;"></div>

                <div id="semaphore-family-actions">
                    <button class="button button-primary" id="semaphore-convert-to-family" disabled>Convert selected tag to family</button>
                    <button class="button" id="semaphore-attach-to-family" disabled>Attach selected tag to current family</button>
                    <button class="button" id="semaphore-detach-from-family" disabled>Detach selected member from current family</button>
...
```html
<div id="mtphr-settings-app" data-id="MtphrPostDuplicatorSettings" data-title="Post Duplicator Settings"><div class="mtphrSettings__container"><div class="mtphrSettings__main" style="max-width: 1000px;"><div data-wp-c16t="true" data-wp-component="Card" class="components-surface components-card mtphrSettings MtphrPostDuplicatorSettings css-c0s3te e19lxcc00" style="position: relative;"><div class="css-10klw3m e19lxcc00"><div data-wp-c16t="true" data-wp-component="CardHeader" class="components-flex components-card__header components-card-header css-1qdoksv e19lxcc00"><div class="mtphrSettings__header"><div class="mtphrSettings__header-content"><h1 data-wp-c16t="true" data-wp-component="Heading" class="components-truncate components-text components-heading mtphrSettings__header-title css-38wvtf e19lxcc00" style="padding: 0px;">Post Duplicator Settings</h1><p class="mtphrSettings__header-description">Configure default duplication settings, manage user permissions, and set advanced options for post duplication.</p></div><div class="mtphrSettings__header-version"><span class="mtphrSettings__version-badge">v3.0.6</span></div></div></div><div class="toolbar"></div><div data-wp-c16t="true" data-wp-component="CardBody" class="components-card__body components-card-body mtphrSettings__form css-9ii361 e19lxcc00"><div class="mtphrSettings__tabs"><div role="tablist" aria-orientation="horizontal" class="components-tab-panel__tabs"><button type="button" aria-selected="true" id="tab-panel-0-general" role="tab" aria-controls="tab-panel-0-general-view" class="components-button components-tab-panel__tabs-item is-active is-next-40px-default-size" data-active-item="true">General</button><button type="button" aria-selected="false" id="tab-panel-0-defaults" role="tab" aria-controls="tab-panel-0-defaults-view" class="components-button components-tab-panel__tabs-item is-next-40px-default-size" tabindex="-1">Defaults</button><button type="button" aria-selected="false" id="tab-panel-0-post_types" role="tab" aria-controls="tab-panel-0-post_types-view" class="components-button components-tab-panel__tabs-item is-next-40px-default-size" tabindex="-1">Post Types</button><button type="button" aria-selected="false" id="tab-panel-0-permissions" role="tab" aria-controls="tab-panel-0-permissions-view" class="components-button components-tab-panel__tabs-item is-next-40px-default-size" tabindex="-1">Permissions</button><button type="button" aria-selected="false" id="tab-panel-0-advanced" role="tab" aria-controls="tab-panel-0-advanced-view" class="components-button components-tab-panel__tabs-item is-next-40px-default-size" tabindex="-1">Advanced</button></div><div id="tab-panel-0-general-view" data-open="true" role="tabpanel" aria-labelledby="tab-panel-0-general" class="components-tab-panel__tab-content" data-enter="true"><div class="mtphrSettings__section"><div data-wp-c16t="true" data-wp-component="Card" class="components-surface components-card mtphrSettings__field mtphrSettings__field--selection css-c0s3te e19lxcc00" style="flex: 1 1 0%;"><div class="css-10klw3m e19lxcc00"><div data-wp-c16t="true" data-wp-component="CardBody" class="components-card__body components-card-body mtphrSettings__field__input-wrapper css-1bamolm e19lxcc00"><div class="components-base-control css-qy3gpb ej5x27r4"><div class="components-base-control__field css-1sf3vf3 ej5x27r3"><label class="components-base-control__label css-2o4jwd ej5x27r2" for="mode">Mode</label><div data-wp-c16t="true" data-wp-component="VStack" class="components-flex components-h-stack components-v-stack css-qq8u2x e19lxcc00"><div class="mtphrSettings__selection-option" role="radio" aria-checked="false" tabindex="0"><div data-wp-c16t="true" data-wp-component="HStack" class="components-flex components-h-stack css-1yxcnbq e19lxcc00"><div class="mtphrSettings__selection-radio"><input type="radio" name="mode" class="mtphrSettings__selection-radio-input" value="basic"></div><div class="mtphrSettings__selection-content"><div class="mtphrSettings__selection-heading">Basic</div><div class="mtphrSettings__selection-description">Quickly duplicate posts based on duplication default settings.</div></div></div></div><div class="mtphrSettings__selection-option mtphrSettings__selection-option--selected" role="radio" aria-checked="true" tabindex="0"><div data-wp-c16t="true" data-wp-component="HStack" class="components-flex components-h-stack css-1yxcnbq e19lxcc00"><div class="mtphrSettings__selection-radio"><input type="radio" name="mode" class="mtphrSettings__selection-radio-input" value="advanced" checked=""></div><div class="mtphrSettings__selection-content"><div class="mtphrSettings__selection-heading">Advanced</div><div class="mtphrSettings__selection-description">Open a modal to adjust duplicated settings for each post before the post is duplicated.</div></div></div></div></div></div></div></div></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div></div></div></div></div></div><div data-wp-c16t="true" data-wp-component="CardFooter" class="components-flex components-card__footer components-card-footer mtphrSettings__footer css-f7p8dt e19lxcc00"><button type="button" class="components-button is-primary">Save Settings</button></div><div class="components-snackbar-list" tabindex="-1" data-testid="snackbar-list"></div></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div></div></div><div class="mtphrSettings__sidebar" style="width: 300px;"><div data-wp-c16t="true" data-wp-component="Card" class="components-surface components-card mtphrSettings__field mtphrSettings__field--group css-c0s3te e19lxcc00" style="flex: 1 1 0%;"><div class="css-10klw3m e19lxcc00"><div data-wp-c16t="true" data-wp-component="CardBody" class="components-card__body components-card-body mtphrSettings__field__input-wrapper css-1bamolm e19lxcc00"><div data-wp-c16t="true" data-wp-component="VStack" class="components-flex components-h-stack components-v-stack css-c4z8vk e19lxcc00"><div data-wp-c16t="true" data-wp-component="Card" class="components-surface components-card mtphrSettings__field mtphrSettings__field--heading css-1co08mm e19lxcc00" style="flex: 1 1 0%; background-color: transparent; margin: 0px;"><div class="css-10klw3m e19lxcc00"><div data-wp-c16t="true" data-wp-component="CardBody" class="components-card__body components-card-body mtphrSettings__field__input-wrapper css-1bamolm e19lxcc00" style="padding: 0px;"><div class="components-base-control css-qy3gpb ej5x27r4"><div class="components-base-control__field css-1sf3vf3 ej5x27r3"><h4 data-wp-c16t="true" data-wp-component="Heading" class="components-truncate components-text components-heading css-19pngs0 e19lxcc00">Documentation</h4></div></div></div></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div></div><div data-wp-c16t="true" data-wp-component="Card" class="components-surface components-card mtphrSettings__field mtphrSettings__field--links css-1co08mm e19lxcc00" style="flex: 1 1 0%; background-color: transparent;"><div class="css-10klw3m e19lxcc00"><div data-wp-c16t="true" data-wp-component="CardBody" class="components-card__body components-card-body mtphrSettings__field__input-wrapper css-1bamolm e19lxcc00" style="padding: 0px;"><div class="components-base-control css-qy3gpb ej5x27r4"><div class="components-base-control__field css-1sf3vf3 ej5x27r3"><ul class="mtphrSettings__links" style="margin: 0px;"><li class="mtphrSettings__links-item"><a href="https://www.metaphorcreations.com/article/post-duplicator/general/duplicating-posts/?ref=post-duplicator" target="_blank" rel="noopener noreferrer">Duplicating Posts</a></li><li class="mtphrSettings__links-item"><a href="https://www.metaphorcreations.com/article/post-duplicator/general/settings/?ref=post-duplicator" target="_blank" rel="noopener noreferrer">Settings</a></li></ul></div></div></div></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div></div></div></div></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div></div><div data-wp-c16t="true" data-wp-component="Card" class="components-surface components-card mtphrSettings__field mtphrSettings__field--group css-1co08mm e19lxcc00" style="flex: 1 1 0%; background-color: transparent;"><div class="css-10klw3m e19lxcc00"><div data-wp-c16t="true" data-wp-component="CardBody" class="components-card__body components-card-body mtphrSettings__field__input-wrapper css-1bamolm e19lxcc00" style="padding: 0px;"><div data-wp-c16t="true" data-wp-component="VStack" class="components-flex components-h-stack components-v-stack css-11fqziw e19lxcc00"><div data-wp-c16t="true" data-wp-component="Card" class="components-surface components-card mtphrSettings__field mtphrSettings__field--ad css-1co08mm e19lxcc00" style="flex: 1 1 0%; background-color: transparent;"><div class="css-10klw3m e19lxcc00"><div data-wp-c16t="true" data-wp-component="CardBody" class="components-card__body components-card-body mtphrSettings__field__input-wrapper css-1bamolm e19lxcc00" style="padding: 0px;"><div class="components-base-control css-qy3gpb ej5x27r4"><div class="components-base-control__field css-1sf3vf3 ej5x27r3"><div style="display: block; line-height: 0;"><a href="https://www.metaphorcreations.com/wordpress-plugins/email-customizer/?campaign=post-duplicator-settings&amp;ref=266" target="_blank" rel="noopener noreferrer" style="display: inline-block;"><img src="http://localhost:8080/wp-content/plugins/post-duplicator/assets/img/marketing/email-customizer.png" alt="Advertisement" style="max-width: 100%; height: auto; display: block;"></a></div></div></div></div></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div></div><div data-wp-c16t="true" data-wp-component="Card" class="components-surface components-card mtphrSettings__field mtphrSettings__field--ad css-1co08mm e19lxcc00" style="flex: 1 1 0%; background-color: transparent;"><div class="css-10klw3m e19lxcc00"><div data-wp-c16t="true" data-wp-component="CardBody" class="components-card__body components-card-body mtphrSettings__field__input-wrapper css-1bamolm e19lxcc00" style="padding: 0px;"><div class="components-base-control css-qy3gpb ej5x27r4"><div class="components-base-control__field css-1sf3vf3 ej5x27r3"><div style="display: block; line-height: 0;"><a href="https://www.metaphorcreations.com/ditty/pricing/?campaign=post-duplicator-settings&amp;ref=266" target="_blank" rel="noopener noreferrer" style="display: inline-block;"><img src="http://localhost:8080/wp-content/plugins/post-duplicator/assets/img/marketing/ditty-everything.png" alt="Advertisement" style="max-width: 100%; height: auto; display: block;"></a></div></div></div></div></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div></div></div></div></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div><div data-wp-c16t="true" data-wp-component="Elevation" class="components-elevation css-jirbkv e19lxcc00" aria-hidden="true"></div></div></div></div></div>
```
...
## PROMPT_12b
Make a quick description to help the user for the page show in `function semaphore_render_dashboard_page()` how to manipulate "All Tags" (Tag Posts) and "Tag Families", add tags, delete tags from famimis, explianening the green color... and so on.
...
<!-- user help on how-to click -->

```

## PROMPT_12a

Simplify the css management and leverage on the existing css, load it in the plugin and manage semaphore-admin-inline from this CSS. Be very precise on what to replace/change inside the code plugin. 
1. The css has to be loaded from `assets/css/bf-semantic-seo-styles.css`
2. The `semaphore_admin_enqueue` needs to be properly refine or change if you need to replace or change the function.


- assets/css/bf-semantic-seo-styles.css
```css
/* semaphore-admin.css */
.semaphore-row-selected {
...
## HUMAN
Make some human code cleaning to be compliant with best practices for coding WP plugin.
...
## PROMPT_11
I have made the resource for the css but rewrite the all plugin with css loading and the changes that you advise in terms of look and feel i ma lazy remember, I just want to cut and paste.
...
## PROMPT_10
Let's work on the look and feel for good.
I have a former css already available mostly for the website or the frontend. How do I integrate this CSS into the semaphore plugin to have a better look and feel.

- bf-semantic-seo-styles.css

```css
/**
 * BF Semantic SEO - Complete Styles
 * 
 * Includes styles for:
 * - Breadcrumbs
 * - Sidebar Widget
 ...
 * - Schema helpers
 * 
 * Theme Color: Zaatar Purple (#4F1993)
 * Version: 1.0.0
 */
...
/* ===========================================================================
   SIDEBAR WIDGET - Zaatar Purple Theme
   =========================================================================== */
...
.semantic-sidebar-widget:hover {
    box-shadow: 0 4px 16px rgba(79, 25, 147, 0.15);
}
...
.semantic-sidebar-widget ul li:hover {
    padding-left: 5px;
    background: rgba(79, 25, 147, 0.03);
    border-radius: 4px;
}
...
.semantic-sidebar-widget .related-tags-section {
    margin-top: 25px;
}

.semantic-sidebar-widget .tag-cloud {
    margin-top: 12px;
}

/* Tag badges - Zaatar purple style */
.tag-badge {
    display: inline-block;
    padding: 7px 14px;
...
.tag-badge:hover {
    ...
    box-shadow: 0 4px 12px rgba(79, 25, 147, 0.3);
}
...
/* Tag badge alternative flat style */
.tag-badge-flat {
    display: inline-block;
    padding: 6px 12px;
    margin: 3px;
...
.bf-debug-box,
.semantic-sidebar-widget .debug-info {
    background: #fff3cd;
    padding: 12px 15px;
    margin-bottom: 15px;
...
/* Spacing utilities */
.bf-mb-10 { margin-bottom: 10px; }
.bf-mb-20 { margin-bottom: 20px; }
.bf-mb-30 { margin-bottom: 30px; }

.bf-mt-10 { margin-top: 10px; }
.bf-mt-20 { margin-top: 20px; }
.bf-mt-30 { margin-top: 30px; }

.bf-p-10 { padding: 10px; }
.bf-p-20 { padding: 20px; }
.bf-p-30 { padding: 30px; }
...
.bf-breadcrumbs a:focus,
.semantic-sidebar-widget a:focus,
.tag-badge:focus,
.related-post-card a:focus {
    outline: 2px solid #4F1993;
    outline-offset: 2px;
}
...
@media (prefers-color-scheme: dark) {
    .semantic-sidebar-widget {
        ...
        border-left-color: #6d2cc4;
        box-shadow: 0 2px 8px rgba(109, 44, 196, 0.2);
    }
    
    .semantic-sidebar-widget h3 {
        color: #9d6fd4;
        border-bottom-color: #6d2cc4;
    }
    
    .semantic-sidebar-widget ul li {
        border-bottom-color: rgba(109, 44, 196, 0.2);
    }
...
.related-post-card:nth-child(1) { animation-delay: 0s; }
.related-post-card:nth-child(2) { animation-delay: 0.1s; }
.related-post-card:nth-child(3) { animation-delay: 0.2s; }
.related-post-card:nth-child(4) { animation-delay: 0.3s; }
.related-post-card:nth-child(5) { animation-delay: 0.4s; }

/* ===========================================================================
   END BF SEMANTIC SEO STYLES
   =========================================================================== */
```
