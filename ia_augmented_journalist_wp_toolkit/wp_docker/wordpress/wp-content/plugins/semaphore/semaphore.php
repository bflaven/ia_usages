...
// Load usage_count sync logic early.
require_once __DIR__ . '/inc/tag-families-usage-sync.php';
...
/*
Then deploy this version, log in as admin on production, hit /wp-admin/?semaphore_repair_usage=1 once, and afterwards you can remove the admin_init block if you want to keep the file clean.
*/
/*
add_action( 'admin_init', function () {
    ...
    // Run once by visiting /wp-admin/?semaphore_repair_usage=1
    if ( isset( $_GET['semaphore_repair_usage'] ) ) {
        semaphore_repair_tag_families_usage();
        wp_die( 'Semaphore usage_count repaired.' );
    }
});

function semaphore_repair_tag_families_usage() {
    global $wpdb;

    $table_families = $wpdb->prefix . 'tag_families';
    $table_tr       = $wpdb->term_relationships;
    $table_tt       = $wpdb->term_taxonomy;
    $table_posts    = $wpdb->posts;

    $sql = "
        UPDATE {$table_families} f
        JOIN (
            SELECT tt.term_id, COUNT(*) AS real_usage
            FROM {$table_tr} tr
            JOIN {$table_tt} tt
...
function semaphore_ajax_list_tags() {
    ...
    $sql_total = "
        ...
        $where
    ";
    $total = (int) $wpdb->get_var( $params ? $wpdb->prepare( $sql_total, ...$params ) : $sql_total );
...
function semaphore_ajax_list_family_members() {
    ...
    // IMPORTANT: use canonical_tag_id as the grouping key for this family
    $sql = $wpdb->prepare(
        "SELECT 
            tag_id,
            tag_label,
...
function semaphore_ajax_detach_from_family() {
    ...
    // IMPORTANT: families are grouped by canonical_tag_id in your data
    $deleted = $wpdb->delete(
        $table,
        array(
            'canonical_tag_id' => $family_id,
