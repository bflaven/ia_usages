...
[file]
streamlit run 004_project_3_python_streamlit_app_chatgpt_api.py
...
# CASE_22 :: 22. Spreadsheet creator
...
def Case_9_DrawMainAppBody():
    st.markdown(f'**Case_9_DrawMainAppBody.**')
...
def Case_10_DrawMainAppBody():
    st.markdown(f'**Case_10_DrawMainAppBody.**')

# CASE_11
# 11. Python to natural language


def Case_11_DrawMainAppBody():
    st.markdown(f'**Case_11_DrawMainAppBody.**')

# CASE_12
# 12. Movie to Emoji(movie_to_emoji)


def Case_12_DrawMainAppBody():

    # PROMPT_INPUT = 'Convert movie titles into emoji.\n\nBack to the Future: 👨👴🚗🕒 \nBatman: 🤵🦇 \nTransformers: 🚗🤖 \nStar Wars: \Tootsie:'
    # PROMPT_INPUT = 'Convert movie titles into emoji.\n\nBack to the Future: 👨👴🚗🕒 \nBatman: 🤵🦇 \nTransformers: 🚗🤖 \nStar Wars: \Tootsie:'
    
    PROMPT_INPUT = '\nThe godfather:'

    st.markdown(f'**Movie to Emoji.**')
    st.markdown('**Example:**')
    st.code(PROMPT_INPUT)

    text_input = st.text_area(
        "Convert to emoji", help=TEXT_HELP_CASE_12_1)

    # Define function to summarize text using OpenAI Codex
    def convert_emoji(text_input):
        # Change to the desired OpenAI model
        model_engine = "text-davinci-003"
    ...
    # Define Streamlit app behavior
    if st.button("Convert Movie to Emoji"):
        output_text = convert_emoji(text_input)
        st.text_area("Emoji produced", output_text)

# CASE_22
# 22. Python bug fixer


def Case_22_DrawMainAppBody():
    

    # PROMPT_INPUT = '##### Fix bugs in the below function\n\n ### Buggy Python\nimport Random\na = random.randint(1,12)\nb = random.randint(1,12)\nfor i in range(10):\n    question = \"What is \"+a+\" x \"+b+\"? \"\n    answer = input(question)\n    if answer = a*b\n        print (Well done!)\n    else:\n        print(\"No.\")\n    \n### Fixed Python'
    
    PROMPT_INPUT = '##### Fix bugs in the below function\n\n def my_func(name, place): \n print(f\'Hello name}! Are you from {place}?\') \n my_func("Prisca", "Paris")'
    

    st.markdown(f'** Python bug fixer.**')
    st.markdown('**Example:**')
    st.code(PROMPT_INPUT)

    text_input = st.text_area(
        "Fix code", help=TEXT_HELP_CASE_22_1)

    # Define function to summarize text using OpenAI Codex
    def fix_code(text_input):
        # Change to the desired OpenAI model
        model_engine = "text-davinci-003"
        prompt = f"{text_input}"
        response = openai.Completion.create(
    ...
    # Define Streamlit app behavior
    if st.button("Fix python code"):
        output_text = fix_code(text_input)
        st.text_area("Code fixed", output_text)
...
def Case_23_DrawMainAppBody():
    ...
    st.markdown(f'**Spreadsheet creator.**')
    st.markdown('**Example:**')
    st.code(PROMPT_INPUT)

    text_input = st.text_area(
        "Create Spreadsheet", help=TEXT_HELP_CASE_23_1)
    ...
    if st.button("Generate spreadsheet"):
        output_text = extract_kws_text(text_input)
        st.text_area("Spreadsheet generated", output_text)

    

# CASE_39
# 39. Notes to summary
def Case_39_DrawMainAppBody():
    st.markdown(f'**Summarize notes.**')
    st.markdown('**Example:**')
    st.code(
        "Tom: Profits up 50%\nJane: New servers are online\nKjel: Need more time to fix software\nJane: Happy to help\nParkman: Beta testing almost done")

    text_input = st.text_area(
        "Convert notes to summary", help=TEXT_HELP_CASE_39_1)
    ...
    # Define Streamlit app behavior
    if st.button("Notes Summarize"):
        output_text = extract_kws_text(text_input)
        st.text_area("Meeting Notes summarized", output_text)
    ...
    if menu == str(option):
        ...
        if i == 9:
            ...
            greatExpender(LABEL_EXPANDER,
                          f"{MENU_SIDEBAR_USECASE_DESCRIPTION_OPTIONS[i]}")
            # MAIN BODY
            Case_9_DrawMainAppBody()
        
        if i == 10:
            greatExpender(LABEL_EXPANDER,
                          f"{MENU_SIDEBAR_USECASE_DESCRIPTION_OPTIONS[i]}")
            # MAIN BODY
            Case_10_DrawMainAppBody()

        if i == 11:
            greatExpender(LABEL_EXPANDER,
                          f"{MENU_SIDEBAR_USECASE_DESCRIPTION_OPTIONS[i]}")
            # MAIN BODY
            Case_11_DrawMainAppBody()

        if i == 12:
            greatExpender(LABEL_EXPANDER,
                          f"{MENU_SIDEBAR_USECASE_DESCRIPTION_OPTIONS[i]}")
            # MAIN BODY
            Case_12_DrawMainAppBody()

        if i == 22:
            greatExpender(LABEL_EXPANDER,
                          f"{MENU_SIDEBAR_USECASE_DESCRIPTION_OPTIONS[i]}")
            # MAIN BODY
            Case_22_DrawMainAppBody()

        if i == 23:
            greatExpender(LABEL_EXPANDER,
                          f"{MENU_SIDEBAR_USECASE_DESCRIPTION_OPTIONS[i]}")
            # MAIN BODY
            Case_23_DrawMainAppBody()
