...
# launch the file
streamlit run 006_ia_video_editing_spacy_entitities_cuepoints_streamlit.py
...
def create_timecode_link(time_ms, unique_id):
    return st.button(
        ...
        on_click=lambda: st.components.v1.html(
            f"""
            <script>
            window.parent.postMessage({{ type: 'seekVideo', time: {time_ms/1000} }}, '*');
            </script>
            """,
            height=0
        )
                        ...
                        if category in analysis_results['entities']['timecodes'] and \
                           entity in analysis_results['entities']['timecodes'][category]:
                            ...
                            for timecode_index, timecode in enumerate(analysis_results['entities']['timecodes'][category][entity].values()):
                                create_timecode_link(timecode[0]['start_time_ms'], f"entity_{category}_{entity}_{timecode_index}")
                                st.markdown(
            ...
            with tab2:
                ...
                for phrase_index, phrase in enumerate(analysis_results['key_phrases']['detected']):
                    ...
                    if phrase in analysis_results['key_phrases']['timecodes']:
                        # In the Key Phrases tab
                        for timecode_index, timecode in enumerate(analysis_results['key_phrases']['timecodes'][phrase].values()):
                            create_timecode_link(timecode[0]['start_time_ms'], f"phrase_{phrase}_{timecode_index}")     
                            st.markdown(
                                    f"📍 Start: {timecode[0]['start_time']} | "
                                    f"End: {timecode[0]['end_time']}"
                                )
