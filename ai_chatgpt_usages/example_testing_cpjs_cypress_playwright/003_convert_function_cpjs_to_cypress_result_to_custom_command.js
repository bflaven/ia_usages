...
// FUNCTION_1 - Convert to a custom command
Cypress.Commands.add("CrawAddWireImage", () => {
    ...
    cy.get("#medium_title")
    .should("be.visible")
    .type(globalValues.MEDIUM_TITLE + globalVariables.RandomString);

    // medium_caption
    cy.get("#medium_caption")
    .should("be.visible")
    .type(globalValues.MEDIUM_CAPTION + " " + globalVariables.RandomString);

    // medium_copyright
    cy.get("#medium_copyright")
    .should("be.visible")
    .type(globalValues.MEDIUM_COPYRIGHT + " " + globalVariables.RandomString);

    // medium_copyrightUrl
    cy.get("#medium_copyrightUrl")
    .should("be.visible")
    .type(globalValues.MEDIUM_COPYRIGHTURL + " " + globalVariables.RandomString);

    cy.get(
    #${globalValues.CSS_WIRE_IMAGE} div div:nth-child(1) div:nth-child(2) div form div:nth-child(2) button:first
    ).click();
});

// FUNCTION_2 - Convert to a custom command
Cypress.Commands.add("RemoveTimeScopeForImage", () => {
    // remove time scope for image
