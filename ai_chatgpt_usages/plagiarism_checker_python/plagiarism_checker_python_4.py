...
[file]
streamlit run plagiarism_checker_python_4.py
...
vectors = vectorize(source_content)
...
def check_plagiarism():
    global s_vectors
    for user_a, text_vector_a in s_vectors:
        new_vectors = s_vectors.copy()
        current_index = new_vectors.index((user_a, text_vector_a))
        del new_vectors[current_index]
        for user_b, text_vector_b in new_vectors:
            sim_score = similarity(text_vector_a, text_vector_b)[0][1]
            user_pair = sorted((user_a, user_b))
            score = (user_pair[0], user_pair[1], sim_score)
...
column_names = ["User_a", "User_b", 'Score']
...
if show:
    ...
    df_show = pd.read_csv(file_output, header=None)
    st.write(df_show)
    ...
    for index, row in df.iterrows():
        ...
        label_student_a = f''+row["User_a"]+''
        label_student_b = f''+row["User_b"]+''
